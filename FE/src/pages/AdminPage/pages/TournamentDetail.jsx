    ...
    : <div style={{display:"grid",gap:10}}>{races.map((race)=>{
      ...
      return (
        <div key={id} role="button" tabIndex={0}
          ...
          style={{borderRadius:12,border:"1px solid rgba(143,100,32,0.16)",background:"rgba(255,250,240,0.96)",padding:"13px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",flexWrap:"wrap",gap:8,transition:"box-shadow 0.15s ease",boxShadow:"0 0 0 transparent"}}
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <p style={{margin:"2px 0 0",fontSize:12,color:"#657086"}}>{fmtDate2(race.scheduledAt??race.ScheduledAt)} · {(race.distance??race.Distance??0)}m · {(race.entriesCount??race.EntriesCount??0)}/{(race.maxParticipants??race.MaxParticipants??12)} ngựa</p>
          </div>
